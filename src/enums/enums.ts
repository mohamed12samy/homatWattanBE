export enum Governorate {
  qahera = "القاهرة",
  giza = "الجيزة",
  alex = "الأسكندرية",
  dakahlia = "الدقهلية",
  sharqia = "الشرقية",
  menoufia = "المنوفية",
  qaliobia = "القليوبية",
  beheira = "البحيرة",
  gharbiya = "الغربية",
  portsaid = "بور سعيد",
  damietta = "دمياط",
  Ismailia = "الإسماعلية",
  suez = "السويس",
  kafr_sheikh = "كفر الشيخ",
  faiyum = "الفيوم",
  bani_suef = "بني سويف",
  matrooh = "مطروح",
  northsinai = "شمال سيناء",
  south_sinai = "جنوب سيناء",
  minya = "المنيا",
  asyut = "أسيوط",
  sohag = "سوهاج",
  qena = "قنا",
  red_sea = "البحر الأحمر",
  luxor = "الأقصر",
  aswan = "أسوان",
  oases = "الواحات",
  wadi = "الوادي الجديد"
}
type GovernmentsMappingType = {
  [key: string]: string;
};
export const GovernmentsMapping : GovernmentsMappingType  =
{
    "القاهرة": "qahera",
    "الجيزة": "giza",
    "الأسكندرية": "alex",
    "الدقهلية": "dakahlia",
    "الشرقية": "sharqia",
    "المنوفية": "menoufia",
    "القليوبية": "qaliobia",
    "البحيرة": "beheira",
    "الغربية": "gharbiya",
    "بور سعيد": "portsaid",
    "دمياط": "damietta",
    "الإسماعلية": "Ismailia",
    "السويس": "suez",
    "كفر الشيخ": "kafr_sheikh",
    "الفيوم": "faiyum",
    "بني سويف": "bani_suef",
    "مطروح": "matrooh",
    "شمال سيناء": "northsinai",
    "جنوب سيناء": "south_sinai",
    "المنيا": "minya",
    "أسيوط": "asyut",
    "سوهاج": "sohag",
    "قنا": "qena",
    "البحر الأحمر": "red_sea",
    "الأقصر": "luxor",
    "أسوان": "aswan",
    "الواحات": "oases",
    "الوادي الجديد": "wadi"

};
export enum GovernoratesCodes {
  qahera = "01",
  alex = "02",
  portsaid = "03",
  suez = "04",
  damietta = "11",
  dakahlia = "12",
  sharqia = "13",
  qaliobia = "14",
  kafr_sheikh = "15",
  gharbiya = "16",
  menoufia = "17",
  beheira = "18",
  Ismailia = "19",
  giza = "21",
  bani_suef = "22",
  faiyum = "23",
  minya = "24",
  asyut = "25",
  sohag = "26",
  qena = "27",
  aswan = "28",
  luxor = "29",
  red_sea = "31",
  wadi = "32",
  matrooh = "33",
  northsinai = "34",
  south_sinai = "35"
}

export const Neighborhoods: any = {
  qahera: {
    qahera1: "حى السلام أول",
    qahera2: "حى السلام ثانى",
    qahera3: "حى المرج",
    qahera4: "حى المطرية",
    qahera5: "حى النزهة",
    qahera6: "حى عين شمس",
    qahera7: "حى مدينة نصر شرق",
    qahera8: "حى مدينة نصر غرب",
    qahera9: "حى مصر الجديدة",
    qahera10: "حى الأزبكية",
    qahera11: "حى الموسكى",
    qahera12: "حى الوايلى",
    qahera13: "حى باب الشعرية",
    qahera14: "حى بولاق",
    qahera15: "حى عابدين",
    qahera16: "حى غرب",
    qahera17: "حى منشأة ناصر",
    qahera18: "حى وسط",
    qahera19: "حى 15 مايو",
    qahera20: "حى البساتين",
    qahera21: "حى التبين",
    qahera22: "حى الخليفة",
    qahera23: "حى السيدة زينب",
    qahera24: "حى المعادى",
    qahera25: "حى المعصرة",
    qahera26: "حى المقطم",
    qahera27: "حى حلوان",
    qahera28: "حى دار السلام",
    qahera29: "حى طرة",
    qahera30: "حى مصر القديمة",
    qahera31: "حى الأميرية",
    qahera32: "حى الزاوية الحمراء",
    qahera33: "حى الزيتون",
    qahera34: "حى الساحل",
    qahera35: "حى الشرابية",
    qahera36: "حى حدائق القبة",
    qahera37: "حى روض الفرج",
    qahera38: "حى شبرا",
    qahera39: "مدينة 15 مايو",
    qahera40: "بدر",
    qahera41: "الشروق",
    qahera42: "القاهرة الجديدة",
    qahera43: "العاصمة الإدارية الجديدة",
    qahera44: "حي الجمالية"
  },
  giza: {
    giza1: "مركز البدرشين",
    giza2: "مركز الصف",
    giza3: "مركز أطفيح",
    giza4: "مركز العياط",
    giza5: "مركز الواحات البحرية",
    giza6: "مركز منشأة القناطر",
    giza7: "مركز أوسيم",
    giza8: "مركز كرداسة",
    giza9: "مركز أبو النمرس",
    giza10: "حى المهندسين",
    giza11: "حى جنوب الجيزة",
    giza12: "حى الدقى",
    giza13: "حى العجوزة",
    giza14: "حى العمرانية",
    giza15: "حى الهرم",
    giza16: "حى شمال الجيزة",
    giza17: "حى بولاق الدكرور",
    giza18: "حى الوراق",
    giza19: "حى إمبابه",
    giza20: "حى المنيب",
    giza21: "حى بين السرايات",
    giza22: "حى الكيت كات",
    giza23: "الشيخ زايد",
    giza24: "حدائق أكتوبر",
    giza25: "السادس من أكتوبر الجديدة",
    giza26: "سفنكس الجديدة",
    giza27: "مركز الحوامدية"
  },
  qaliobia: {
    qaliobia1: "مركز بنها",
    qaliobia2: "مركز قليوب",
    qaliobia3: "مركز القناطر الخيرية",
    qaliobia4: "مركز الخانكة",
    qaliobia5: "مركز كفر شكر",
    qaliobia6: "مركز طوخ",
    qaliobia7: "مركز شبين القناطر",
    qaliobia8: "مدينة العبور",
    qaliobia9: "شبرا الخيمة أول",
    qaliobia10: "شبرا الخيمة ثاني"
  },
  alex: {
    alex1: "قسم باب شرق",
    alex2: "قسم محرم بك",
    alex3: "قسم كرموس",
    alex4: "قسم العطارين",
    alex5: "قسم الرمل",
    alex6: "قسم سيدي جابر",
    alex7: "قسم مينا البصل",
    alex8: "قسم اللبان",
    alex9: "قسم المنشية",
    alex10: "قسم المنتزة",
    alex11: "قسم الجمرك",
    alex12: "قسم العامرية",
    alex13: "قسم الدخيلة",
    alex14: "مركز برج العرب"
  },
  beheira: {
    beheira1 : "مركز دمنهور",
    beheira2 : "مركز كفر الدوار",
    beheira3 : "مركز رشيد",
    beheira4 : "مركز إدكو",
    beheira5 : "مركز أبو المطامير",
    beheira6 : "مركز أبو حمص",
    beheira7 : "مركز الدلنجات",
    beheira8 : "مركز المحمودية",
    beheira9 : "مركز الرحمانية",
    beheira10 : "مركز إيتاى البارود",
    beheira11 : "مركز حوش عيسى",
    beheira12 : "مركز شبراخيت",
    beheira13 : "مركز كوم حماده",
    beheira14 : "مركز بدر",
    beheira15 : "مركز وادى النطرون"
  },
  sharqia: {
    sharqia1 : "الزقازيق",
    sharqia2 : "منيا القمح",
    sharqia3 : "بلبيس",
    sharqia4 : "مشتول السوق",
    sharqia5 : "أبو حماد",
    sharqia6 : "ههيا",
    sharqia7 : "أبو كبير",
    sharqia8 : "فاقوس",
    sharqia9 : "الإبراهيمية",
    sharqia10 : "ديرب نجم",
    sharqia11 : "كفر صقر",
    sharqia12 : "أولاد صقر",
    sharqia13 : "الحسينية",
    sharqia14 : "العاشر من رمضان",
    sharqia15 : "الصالحية الجديدة",
    sharqia16 : "القرين"
  },
  menoufia: {
    menoufia1 : "شبين الكوم",
    menoufia2 : "مدينة السادات",
    menoufia3 : "منوف",
    menoufia4 : "أشمون",
    menoufia5 : "الباجور",
    menoufia6 : "قويسنا",
    menoufia7 : "بركة السبع",
    menoufia8 : "تلا",
    menoufia9 : "الشهداء",
    menoufia10 : "سرس الليان"
  },
  gharbiya: {
    gharbiya1 : "مركز طنطا",
    gharbiya2 : "مركز المحلة الكبرى",
    gharbiya3 : "مركز كفر الزيات",
    gharbiya4 : "زفتى",
    gharbiya5 : "السنطة",
    gharbiya6 : "قطور",
    gharbiya7 : "بسيون",
    gharbiya8 : "سمنود"
  },
  kafr_sheikh: {
    kafr_sheikh1 : "مركز كفر الشيخ",
    kafr_sheikh2 : "مركز دسوق",
    kafr_sheikh3 : "مركز فوه",
    kafr_sheikh4 : "مركز مطوبس",
    kafr_sheikh5 : "مركز البرلس",
    kafr_sheikh6 : "مركز الحامول",
    kafr_sheikh7 : "مركز بيلا",
    kafr_sheikh8 : "مركز الرياض",
    kafr_sheikh9 : "مركز سيدى سالم",
    kafr_sheikh10 : "مركز قلين"
  },
  dakahlia: {
    dakahlia1 : "مركز المنصورة",
    dakahlia2 : "مركز طلخا",
    dakahlia3 : "مركز ميت غمر",
    dakahlia4 : "مركز دكرنس",
    dakahlia5 : "مركز أجا",
    dakahlia6 : "مركز منية النصر",
    dakahlia7 : "مركز السنبلاوين",
    dakahlia8 : "مركز بنى عبيد",
    dakahlia9 : "مركز المنزلة",
    dakahlia10 : "مركز تمى الأمديد",
    dakahlia11 : "مركز الجمالية",
    dakahlia12 : "مركز شربين",
    dakahlia13 : "مركز المطرية",
    dakahlia14 : "مركز بلقاس",
    dakahlia15 : "مركز ميت سلسيل",
    dakahlia16 : "مركز محلة دمنة",
    dakahlia17 : "مركز نبروه"
  },
  Ismailia: {
    Ismailia1 : "مركز الإسماعيلية",
    Ismailia2 : "فايد",
    Ismailia3 : "القنطرة شرق",
    Ismailia4 : "القنطرة غرب",
    Ismailia5 : "التل الكبير",
    Ismailia6 : "أبو صوير",
    Ismailia7 : "القصاصين",
    Ismailia8 : "قسم أول الإسماعيلية",
    Ismailia9 : "قسم ثانى الإسماعيلية"
  },
  northsinai: {
    northsinai1 : "العريش",
    northsinai2 : "الشيخ زويد",
    northsinai3 : "رفح",
    northsinai4 : "بئر العبد",
    northsinai5 : "الحسنة",
    northsinai6 : "نخل"
  },
  bani_suef: {
    bani_suef1 : "بنى سويف",
    bani_suef2 : "الواسطى",
    bani_suef3 : "ناصر",
    bani_suef4 : "أهناسيا",
    bani_suef5 : "ببا",
    bani_suef6 : "سمسطا",
    bani_suef7 : "الفشن"
  },
  faiyum: {
    faiyum1 : "مركز الفيوم",
    faiyum2 : "مركز طاميه",
    faiyum3 : "مركز سنورس",
    faiyum4 : "مركز إطسا",
    faiyum5 : "مركز إبشواى",
    faiyum6 : "مركز يوسف الصديق"
  },
  minya: {
    minya1 : "مركز المنيا",
    minya2 : "العدوة",
    minya3 : "مغاغه",
    minya4 : "بنى مزار",
    minya5 : "مطاى",
    minya6 : "سمالوط",
    minya7 : "أبو قرقاص",
    minya8 : "ملوى",
    minya9 : "ديرمواس"
  },
  asyut: {
    asyut1 : "مركز أسيوط",
    asyut2 : "ديروط",
    asyut3 : "منفلوط",
    asyut4 : "القوصيه",
    asyut5 : "أبنوب",
    asyut6 : "أبو تيج",
    asyut7 : "الغنايم",
    asyut8 : "ساحل سليم",
    asyut9 : "البداري",
    asyut10 : "صدفا",
    asyut11 : "الفتح",
    asyut12 : "قسم أسيوط أول",
    asyut13 : "قسم أسيوط ثاني"
  },
  wadi: {
    wadi1 : "الخارجه",
    wadi2 : "باريس",
    wadi3 : "الداخله",
    wadi4 : "الفرافرة",
    wadi5 : "بلاط"
  },
  sohag: {
    sohag1 : "مركز سوهاج",
    sohag2 : "أخميم",
    sohag3 : "البلينا",
    sohag4 : "المراغة",
    sohag5 : "المنشأة",
    sohag6 : "دار السلام",
    sohag7 : "جرجا",
    sohag8 : "جهينه",
    sohag9 : "ساقلته",
    sohag10 : "طما",
    sohag11 : "طهطا",
    sohag12 : "العسيرات",
    sohag13 : "حي شرق",
    sohag14 : "حي غرب"
  },
  qena: {
    qena1 : "قنا",
    qena2 : "أبوتشت",
    qena3 : "نجع حمادى",
    qena4 : "دشنا",
    qena5 : "الوقف",
    qena6 : "قفط",
    qena7 : "نقادة",
    qena8 : "قوص",
    qena9 : "فرشوط"
  },
  luxor: {
    luxor1 : "مركز الأقصر",
    luxor2 : "الزينية",
    luxor3 : "البياضية",
    luxor4 : "القرنة",
    luxor5 : "أرمنت",
    luxor6 : "الطود",
    luxor7 : "إسنا"
  },
  aswan: {
    aswan1 : "أسوان",
    aswan2 : "دراو",
    aswan3 : "كوم أمبو",
    aswan4 : "نصر النوبة",
    aswan5 : "إدفو",
    aswan6 : "أسوان الجديدة",
    aswan7 : "توشكى الجديدة",
    aswan8 : "أبو سمبل"
  },
  portsaid: {
    portsaid1 : "بور فؤاد",
    portsaid2 : "حي الشرق",
    portsaid3 : "حي العرب",
    portsaid4 : "حي المناخ",
    portsaid5 : "حي الضواحي",
    portsaid6 : "حي الجنوب",
    portsaid7 : "حي الزهور",
    portsaid8 : "حي غرب"
  },
  suez: {
    suez1 : "حي السويس",
    suez2 : "حي الأربعين",
    suez3 : "حي عتاقة",
    suez4 : "حي الجناين",
    suez5 : "حي فيصل"
  },
  south_sinai: {
    south_sinai1 : "أبو رديس",
    south_sinai2 : "أبو زنيمة",
    south_sinai3 : "نويبع",
    south_sinai4 : "طابا",
    south_sinai5 : "رأس سدر",
    south_sinai6 : "دهب",
    south_sinai7 : "شرم الشيخ",
    south_sinai8 : "سانت كاترين",
    south_sinai9 : "الطور"
  },
  red_sea: {
    red_sea1 : "رأس غارب",
    red_sea2 : "الغردقة",
    red_sea3 : "القصير",
    red_sea4 : "سفاجا",
    red_sea5 : "مرسى علم",
    red_sea6 : "شلاتين",
    red_sea7 : "مدينة حلايب"
  },
  damietta:{
    damietta1:"مركز دمياط",
    damietta2:"مركز فارسكور",
    damietta3:"مركز كفر سعد",
    damietta4:"مركز الزرقا",
    damietta5:"مركز كفر البطيخ"
  },
  matrooh: {
    matrooh1:"مركز مرسى مطروح",
    matrooh2:"مركز الحمام",
    matrooh3:"مركز العلمين",
    matrooh4:"مركز الضبعة",
    matrooh5:"مركز النجيلة",
    matrooh6:"مركز سيدى برانى",
    matrooh7:"مركز السلوم",
    matrooh8:"مركز سيوة"
  },
  
};

export enum UsereRoles {
  admin = "admin",
  governorator = "governorator",
  departmentHead = "departmentHead",
  adminViewer = "adminViewer" 
}

export const enum Knew {
  socialMedia  = "وسائل التواصل الاجتماعي والميديا",
  friends = "الاصدقاء والمعارف",
  activities = "انشطته ومبادراته وخدماته"
}


export const countries = [
  "السعودية",
  "الكويت",
  "الإمارات",
  "العراق",
  "الأردن",
  "ليبيا",
  "إيطاليا",
  "ألمانيا",
  "أمريكا",
  "قطر",
] as const;


export const election_sections = [
  "ﻣﺠﻠﺲ ﻧﻮاب",
  "ﻣﺠﻠﺲ اﻟﺸﻴﻮخ",
  "اﻻﻧﺪﻳﺔ وﻣﺮاﻛﺰ اﻟﺸﺒﺎب",
  "ﻧﻘﺎﺑﺎت",
  "ﻣﺠﺎﻟﺲ ﻣﺤﻠﻴﺔ",
  "اﺗﺤﺎدات رﻳﺎﺿﻴﺔ",
  "اﺗﺤﺎدات ﻃﻼﺑﻴﺔ",
  "غرف / اﺗﺤﺎدات",
  "أخرى"
] as const;

export const union = [
  "نقابة المحامين المصرية",
  "نقابة الصحفيين",
  "نقابة المهندسين",
  "نقابة الأطباء البشريين",
  "نقابة أطباء الأسنان",
  "نقابة الصيادلة",
  "نقابة الأطباء البيطريين",
  "نقابة الزراعيين",
  "نقابة المعلمين",
  "نقابة السينمائيين",
  "نقابة المهن التمثيلية",
  "نقابة المهن الموسيقية",
  "نقابة التجاريين",
  "نقابة العلميين",
  "نقابة الاجتماعيين",
  "نقابة المهن الفنية التطبيقية",
  "نقابة مصممى الفنون التطبيقية",
  "نقابة الفنانين التشكيليين",
  "نقابة التمريض",
  "نقابة المرشدين السياحيين",
  "نقابة محفظى القرآن الكريم",
  "نقابة الرياضيين",
  "نقابة مستخلصي الجمارك",
  "نقابة العلاج الطبيعي",
  "نقابة المبرمجين المصريين",
  "نقابة الطيارين المدنيين المصرية",
  "نقابة التأهيل الحركي والاصابات",
  "نقابة الاعلاميين",
  "نقابة الاثريين",
  "نقابة المترجمين",
  "نقابة الأئمة والدعاه",
  "أخرى"
] as const;